      ...
      <div className="cont">
        ...
        <FadeUp className="mt-10">
          <Accordion type="multiple" className="relative overflow-hidden rounded-3xl px-6 py-0.5">
            <div className="absolute inset-0 -z-10 rounded-3xl border-2 border-brand/15 [mask-image:linear-gradient(to_bottom,black,transparent)]" />
            ...
            {faqsList.map((faq, i) => (
              <AccordionItem
                value={faq.id}
                key={faq.id}
                className={cn('border-foreground/10 py-2.5', {
                  'border-b-0': i === faqsList.length - 1
                })}
              >
                <AccordionTrigger className="text-lg [&>svg]:text-muted-foreground">
                  {faq.title}
                </AccordionTrigger>
